...
class LinkData(Dataset):
    ...
    def transform_batch(self, track_pairs):
        # 批量处理x1和x2轨迹对
        batch_x1 = []
        batch_x2 = []
        for x1, x2 in track_pairs:
            # 对每一对轨迹进行转换
            transformed_x1, transformed_x2 = self.transform(x1, x2)
            batch_x1.append(transformed_x1)
            batch_x2.append(transformed_x2)
...
if __name__ == '__main__':
    dataset = LinkData(
        root=r'E:\shuju2\数据集\MOT20\train',
        mode='train'
    )
